...
object WorkshopMapper {
    ...
    fun fromCursor(cursor: Cursor): Workshop {
        ...
        val lng = cursor.getDouble(cursor.getColumnIndexOrThrow(WorkshopDbHelper.COLUMN_LNG))
        val phone = cursor.getString(cursor.getColumnIndexOrThrow(WorkshopDbHelper.COLUMN_PHONE))
        val address =
                cursor.getString(cursor.getColumnIndexOrThrow(WorkshopDbHelper.COLUMN_ADDRESS))
        val openTime =
                cursor.getString(cursor.getColumnIndexOrThrow(WorkshopDbHelper.COLUMN_OPEN_TIME))
        val closeTime =
                cursor.getString(cursor.getColumnIndexOrThrow(WorkshopDbHelper.COLUMN_CLOSE_TIME))
        val is24h = cursor.getInt(cursor.getColumnIndexOrThrow(WorkshopDbHelper.COLUMN_IS_24H)) == 1
        val ratingAvg =
                cursor.getDouble(cursor.getColumnIndexOrThrow(WorkshopDbHelper.COLUMN_RATING_AVG))
        val ratingCount =
                cursor.getInt(cursor.getColumnIndexOrThrow(WorkshopDbHelper.COLUMN_RATING_COUNT))
        val source = cursor.getString(cursor.getColumnIndexOrThrow(WorkshopDbHelper.COLUMN_SOURCE))
        val createdAt =
                cursor.getString(cursor.getColumnIndexOrThrow(WorkshopDbHelper.COLUMN_CREATED_AT))
